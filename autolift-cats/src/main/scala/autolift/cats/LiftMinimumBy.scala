...
trait LiftMinimumByContext{
  def liftMaxBy[A, B : Order](f: A => B) = new LiftedMinimumBy(f)
}

trait LiftMinimumByExport{
  implicit def mkMaxBy[Obj, Fn](implicit lift: CatsLiftMinimumBy[Obj, Fn]): CatsLiftMinimumBy.Aux[Obj, Fn, lift.Out] = lift
}
